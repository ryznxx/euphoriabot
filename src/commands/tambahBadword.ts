...
export const tambahBadword: Command = {
  ...
  execute: async (args: string, jid: string, sender: string) => {
    ...
    if (!badwordWasAdded) {
      await sendMessage(
        {
          text: `${coded(args.split(",").toString())} ${error.badwordExists}`,
        },
        sender
      );
      return;
    ...
    if (status) {
      await sendMessage(
        {
          text: `${coded(args.split(",").toString())} ${error.badwordAddSuccess}`,
        },
        sender
      );
    } else {
      await sendMessage(
        {
          text: `${coded(args.split(",").toString())} ${error.badwordAddFailed}`,
        },
        sender
      );
    }
